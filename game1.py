...
def init_game():
    ...
    score = 0
    game_over = False
    return player, balls, ball_spawn_timer, score, game_over

# Создаем игровые объекты
player, balls, ball_spawn_timer, score, game_over = init_game()
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        # Обработка рестарта по нажатию R (даже если игра не окончена)
    ...
    # Если игра не окончена - обновляем игровой мир
    if not game_over:
    ...
    # 2. Рисуем игровые объекты (только если игра не окончена)
    if not game_over:
        player.draw(screen)
        for ball in balls:
            ball.draw(screen)
    ...
    if game_over:
        game_over_text = font.render("GAME OVER! Нажмите R для рестарта", True, RED)
